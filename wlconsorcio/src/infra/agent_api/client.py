...
def choose_handoff_queue(
    ...
    try:
        ...
        print(f"Falha ao escolher fila de handoff, usando fallback: {e}")
        return fallback
...
def generate_free_error_message(agent_name: str, openai_api_key: str | None = None) -> str:
    ...
    try:
        ...
        print(f"Fallback de mensagem de erro também falhou: {e}")
...
def sincronizar_metadados_contato(target_id: str, metadata: dict) -> None:
    ...
    aparecer no Agent Console (Target.metadata). Best-effort: uma falha aqui não deve derrubar a conversa."""
    try:
        httpx.patch(
        ...
        print(f"Falha ao sincronizar metadados do contato {target_id}: {e}")
...
def resetar_metadados_contato(target_id: str) -> None:
    ...
    try:
        ...
        print(f"Falha ao resetar metadados do contato {target_id}: {e}")
...
def bloquear_campanhas_contato(target_id: str) -> None:
    ...
    try:
        ...
        print(f"Falha ao bloquear campanhas do contato {target_id}: {e}")
...
def record_message_logs(logs: list[dict]) -> None:
    ...
    try:
        ...
        print(f"Falha ao gravar MessageLog: {e}")
...
def update_rag_document_status(
    ...
    try:
        ...
        print(f"Falha ao atualizar status do RagDocument {rag_document_id}: {e}")
