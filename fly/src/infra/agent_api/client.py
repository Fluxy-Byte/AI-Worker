...
def choose_handoff_queue(
    ...
    try:
        ...
        print(f"[fly] Falha ao escolher fila de handoff, usando fallback: {e}")
        return fallback
...
def generate_free_error_message(agent_name: str, openai_api_key: str | None = None) -> str:
    ...
    try:
        ...
        print(f"[fly] Fallback de mensagem de erro também falhou: {e}")
...
def record_message_logs(logs: list[dict]) -> None:
    ...
    try:
        ...
        print(f"[fly] Falha ao gravar MessageLog: {e}")
...
def sincronizar_metadados_contato(target_id: str, metadata: dict) -> None:
    ...
    conversa (hoje: o telefone informado manualmente, ver
    tools.registrar_telefone_contato) pro Agent-Api, pra persistir entre
    sessões e aparecer no Agent Console (Target.metadata) — mesmo padrão que
    o axel já usa. Best-effort: uma falha aqui não deve derrubar a conversa."""
    try:
        httpx.patch(
        ...
        print(f"[fly] Falha ao sincronizar metadados do contato {target_id}: {e}")
