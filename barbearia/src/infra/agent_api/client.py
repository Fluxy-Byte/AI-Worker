...
def choose_handoff_queue(
    ...
    try:
        ...
        print(f"[barbearia] Falha ao escolher fila de handoff, usando fallback: {e}")
        return fallback
...
def generate_free_error_message(agent_name: str) -> str:
    ...
    try:
        ...
        print(f"[barbearia] Fallback de mensagem de erro também falhou: {e}")
...
def update_rag_document_status(
    ...
    try:
        ...
        print(f"[barbearia] Falha ao atualizar status do RagDocument {rag_document_id}: {e}")
