...
APP_NAME = os.getenv("GOOGLE_ADK_APP_NAME", "julia")

# Chaves de state que o agente pode acumular via atualizar_nome_cliente/
# atualizar_cidade_cliente/registrar_disponibilidade_contato — sincronizadas
# de volta pro Agent-Api (Target.metadata) a cada turno, pra persistir entre
# sessões (mesmo padrão que o axel já usa).
CHAVES_METADATA = ("nome", "cidade", "dia_preferido", "horario_preferido")
...
async def _abrir_sessao(session_service, user_id: str, session_id: str) -> None:
    ...
    if sessao is not None:
        print(f"[julia] [session={session_id} user={user_id}] sessao ADK existente reutilizada")
        return

    try:
        await session_service.create_session(app_name=APP_NAME, user_id=user_id, session_id=session_id)
        print(f"[julia] [session={session_id} user={user_id}] sessao ADK nova criada")
    except AlreadyExistsError:
        print(f"[julia] [session={session_id} user={user_id}] sessao ADK ja existia (race no create)")
...
async def _executar(pergunta: str, user_id: str, session_id: str, agent_config: dict, target_info: dict) -> ResultadoResposta:
    ...
    print(f"[julia] [session={session_id} user={user_id}] _executar: abrindo session_service")
    async with get_session_service() as session_service:
        ...
        print(
            f"[julia] [session={session_id} user={user_id}] montando agent "
        ...
        async for event in runner.run_async(user_id=user_id, session_id=session_id, new_message=mensagem):
            ...
            if calls:
                ...
                print(f"[julia] [session={session_id} user={user_id}] tool call: {nomes}")
            if event.is_final_response() and event.content and event.content.parts:
                resposta_final = event.content.parts[0].text or resposta_final

        print(
            f"[julia] [session={session_id} user={user_id}] loop do runner terminou, "
        ...
        if sessao_final is not None:
            ...
            print(
                f"[julia] [session={session_id} user={user_id}] state final: "
        ...
        if handoff_requested or closing_requested:
            ...
            print(f"[julia] [session={session_id} user={user_id}] sessao ADK apagada (handoff/closing)")

        print(f"[julia] [session={session_id} user={user_id}] _executar retornando")
