...
def choose_handoff_queue(
    ...
    try:
        ...
        print(f"[max] Falha ao escolher fila de handoff, usando fallback: {e}")
        return fallback
...
def generate_free_error_message(agent_name: str, openai_api_key: str | None = None) -> str:
    ...
    try:
        ...
        print(f"[max] Fallback de mensagem de erro também falhou: {e}")
...
def sincronizar_metadados_contato(target_id: str, metadata: dict) -> None:
    ...
    try:
        ...
        print(f"[max] Falha ao sincronizar metadados do contato {target_id}: {e}")
...
def record_message_logs(logs: list[dict]) -> None:
    ...
    try:
        ...
        print(f"[max] Falha ao gravar MessageLog: {e}")
...
def update_rag_document_status(
    ...
    try:
        ...
        print(f"[max] Falha ao atualizar status do RagDocument {rag_document_id}: {e}")
