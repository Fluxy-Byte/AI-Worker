"""
Consumidor da fila dedicada do agente Fly (`task.agent.fly.create`) — este
worker atende só o Agent "Fly" (escolhido pela env `AGENT_NAME`, mesmo valor
do `Agent.name` cadastrado no Agent Console). Ver
...
O acesso a clientes/OS/serviços é resolvido pelo telefone do contato (ver
src/services/adk/fluxy_auth.py), não por nada deste payload — a personalidade
abaixo só afeta tom de voz e mensagens operacionais (erro, encerramento,
fora de expediente etc.). Sem RAG: ragEnabled é ignorado mesmo se vier ligado
na config do agente.
...

{
  ...
  "agent": {
    ...
    "personality",                  # só usado aqui — tom de voz do agente
  },
...
if not _RAW_AGENT_NAME:
    raise RuntimeError(
        "AGENT_NAME não definida — este worker precisa saber qual Agent "
        "atende (mesmo nome cadastrado no Agent Console)."
    )
...
def _log(payload: dict, msg: str) -> None:
    ...
    print(f"[fly] [session={session.get('id')} wa={target.get('waId')}] {msg}")
...
def _on_message(channel, method, properties, body):
    ...
    try:
        ...
        _log(payload, f"ERRO NAO TRATADO ao processar mensagem do agente Fly: {e}")
        print(traceback.format_exc())
