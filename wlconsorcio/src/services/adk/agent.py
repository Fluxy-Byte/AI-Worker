...
from src.services.adk.tools import (
    atualizar_condicoes_de_pagar_parcelas_remanescentes_cliente,
    atualizar_imovel_de_garantia_cliente,
    atualizar_nome_cliente,
    atualizar_restricao_no_nome_cliente,
    atualizar_tem_um_avalista_cliente,
    encerrar_conversa,
...
## Sua Personalidade:
{personality_block} # Personalidade definida no agent console

## Formas de agir com o primeiro contato ou o contato que ja e cliente:
{fluxo_block}

## Roteiro dos dados que você deve coletar:
{coleta_dados_block}

## Metadados do contato:
{dados_conhecidos_block}

## Informação complementar para responder o usuario
{rag_block}
...
- atualizar_nome_cliente / atualizar_restricao_no_nome_cliente /
  atualizar_imovel_de_garantia_cliente / atualizar_tem_um_avalista_cliente /
  atualizar_condicoes_de_pagar_parcelas_remanescentes_cliente: chame cada uma
  assim que o contato informar ou confirmar o dado correspondente (não
  precisa perguntar de novo o que já estiver na seção "Dados já registrados" acima).
...
TRANSFER_TO_QUEUE = """
Chame a solicitar_atendimento_humano assim que os dados obrigatórios
...
END_CONVERSATION="""
Chame a funcionalidade encerrar_conversa quando o contato se despedir, confirmar que não
...
Antes de encaminhar o atendimento, confirme estes 5 dados sobre o contato,
um de cada vez (pule qualquer um que já esteja em "Dados já registrados"
acima):

1. Nome da pessoa que estamos conversando
2. Se possui restrição no nome que possa atrapalhar
3. Se tem algum imóvel que pretende oferecer como garantia
4. Se possui um avalista disponível para a operação
5. Se tem condições para pagar as parcelas remanescentes do consórcio

Assim que o contato informar cada dado, chame a ferramenta correspondente
imediatamente (atualizar_nome_cliente, atualizar_restricao_no_nome_cliente,
atualizar_imovel_de_garantia_cliente, atualizar_tem_um_avalista_cliente,
atualizar_condicoes_de_pagar_parcelas_remanescentes_cliente).

Assim que TODOS os 5 dados estiverem confirmados (já registrados ou
...
LABELS_METADATA = {
    "nome": "Nome",
    "restricao_no_nome": "Restrição no nome",
    "imovel_de_garantia": "Imóvel de garantia",
    "tem_um_avalista": "Possui avalista",
    "condicoes_de_pagar_parcelas_remanescentes": "Condições de pagar parcelas remanescentes",
}
...
def build_agent(agent_info: dict, target_info: dict | None = None) -> Agent:
    ...
    nome = agent_info.get("name") or "WL Consorcios"
    ...
    instruction = BASE_INSTRUCTION.format(
        ...
        rag_block=rag_block,
        transfer_to_quere=TRANSFER_TO_QUEUE,
    ...
    return Agent(
        ...
        description=f"Canal de comunicação da WL Consorcios.",
