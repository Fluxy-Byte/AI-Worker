...
if not _RAW_AGENT_NAME:
    raise RuntimeError(
        "AGENT_NAME não definida — cada instância do worker da barbearia atende um único "
...
def _log(payload: dict, msg: str) -> None:
    ...
    print(f"[barbearia] [session={session.get('id')} wa={target.get('waId')}] {msg}")
...
def _on_message(channel, method, properties, body):
    ...
    try:
        ...
        # Uma tarefa de envio por parte — quando o agente responde com mais de uma,
        # cada uma vira uma bolha separada no WhatsApp. A sessão só é liberada
        # (finishesProcessing) na última.
